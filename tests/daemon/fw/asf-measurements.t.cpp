...
BOOST_AUTO_TEST_CASE(Basic)
{
  ...
  BOOST_CHECK_EQUAL(stats.getRtt(), rtt.count());
  BOOST_CHECK_EQUAL(stats.getSrtt(), rtt.count());
  ...
  BOOST_CHECK_EQUAL(stats.getRtt(), higherRtt.count());
  BOOST_CHECK_GT(stats.getSrtt(), rtt.count());
  BOOST_CHECK_LT(stats.getSrtt(), higherRtt.count());
...
BOOST_FIXTURE_TEST_CASE(Basic, UnitTestTimeFixture)
{
  ...
  BOOST_CHECK_THROW(info.setTimeoutEvent(id, interestName), std::runtime_error);

  // Receive Data
  shared_ptr<Interest> interest = std::make_shared<Interest>(interestName);
  shared_ptr<pit::Entry> pitEntry = std::make_shared<pit::Entry>(*interest);
  std::shared_ptr<DummyFace> face = std::make_shared<::nfd::tests::DummyFace>();
  ...
  BOOST_CHECK_CLOSE(info.getRtt(), rtt.count(), 1);
  BOOST_CHECK_CLOSE(info.getSrtt(), rtt.count(), 1);
  ...
  // Receiving a timeout for a name that is not a prefix of the scheduled RTO Interest name
  // should throw an exception
  info.setTimeoutEvent(id, interestName);
  BOOST_CHECK_THROW(info.recordTimeout(ndn::Name("/ndn/wrong-name")), std::runtime_error);
}
